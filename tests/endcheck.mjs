...
//  • новые игры подгружаются на старт+110 (конец+5) ДАЖЕ когда newgames.on=false, один раз на игру;
...
console.log('\n===== 2) auto-update OFF, конец+5 (>110м) -> новые игры подгружены =====');
{
  let st={tree:{autocfg:{results:{on:false,after:[],times:[],last:0},newgames:{on:false,last:0}},
  ...
  globalThis.fetch=makeFetch(st);
  let r=await RC();
  ok((r.added||0)>0,'новые игры подгружены на конец+5 при newgames.on=false (added '+(r.added||0)+')');
  ok(st.tree.autocfg.newgames.on===false,'тумблер newgames остался выключенным');
  ok(st.tree.autocfg.endchk&&st.tree.autocfg.endchk.ngLast>0,'отметка endchk.ngLast выставлена (троттлинг)');
  // 2b) повторный прогон тут же — НЕ дёргает повторно (троттлинг)
