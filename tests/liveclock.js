...
ok(hl.html.indexOf('זמן המשחק')<0,'перерыв: подписи «זמן המשחק» нет');
...
// ESPN «23'» = секундомер 22:xx (минута−1), минус калибровка 24с -> 22:00−24с = 21:36
ok(li.html.indexOf('⏱')>=0&&li.html.indexOf('21:36')>=0&&li.color.indexOf('red')>=0&&li.live===true,'идёт игра → ⏱ 21:36 (минута−1 − калибровка) красным (live)');
...
ok(li5.html.indexOf('21:41')>=0,'докрут секунд: +5с → 21:41');
const li59=S.liveLabel({st:'STATUS_FIRST_HALF',clk:"23'",ts:now0},now0+90000);
ok(li59.html.indexOf('22:35')>=0,'докрут секунд ограничен 59 -> 22:59−24с = 22:35 (ждём следующую минуту)');
...
ok(/class="liveclk"/.test(fh)&&fh.indexOf('28:')>=0,'свежий live → liveclk 28:SS (30−1 минута − калибровка 24с)');
...
// fallback зависит от режима: доска (board) — без иконки; להמр — с надписью «המשחק הסתיים»
const noLive={dt:dt,settled:false,live:null};
ok(/data-over="המשחק הסתיים"/.test(S.liveClockHtml(noLive,'',false)),'להמר fallback: расчётный с надписью «המשחק הסתיים»');
ok(/data-board="1"/.test(S.liveClockHtml(noLive,'',true)),'доска fallback: board-режим');
...
// доска: идущая игра → минута:секунды + «זמן המשחק»
...
ok(/class="liveclk"/.test(bh2)&&bh2.indexOf('65:')>=0&&bh2.indexOf('זמן המשחק')>=0,'доска: идёт игра → 65:SS (67−1 − калибровка) + זמן המשחק');
