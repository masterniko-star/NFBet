...
ok(S.gameClockState(start,false,start-25*60*60000).html==='','>24ч до старта → пусто (часы не показаны)');
...
ok(/⏱ 59:59/.test(S.gameClockState(start,false,start-(59*60000+59000)).html),'за ~час → ⏱ 59:59 (MM:SS, <1ч)');
// ≥1ч до старта — формат с часами HH:MM:SS, отсчёт открыт уже за 24ч
ok(/⏱ 01:30:00/.test(S.gameClockState(start,false,start-90*60000).html),'за 1.5ч → ⏱ 01:30:00 (часы)');
ok(/⏱ 20:00:00/.test(S.gameClockState(start,false,start-20*60*60000).html),'за 20ч → ⏱ 20:00:00');
ok(S.gameClockState(start,false,start-90*60000).html.indexOf('המשחק מתחיל בעוד')>=0,'за 1.5ч: подпись «המשחק מתחיל בעוד»');
