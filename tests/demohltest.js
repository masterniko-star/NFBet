...
console.log('\n===== non-owner view: demo highlighted too, no badge =====');
const playerHtml=render('dm'); // смотрит обычный игрок (сам демо) -> own=false
ok((playerHtml.match(/class="unpaidName"/g)||[]).length===1,'демо подсвечен и у обычного участника (подсветка для всех)');
ok(playerHtml.indexOf('\u05de\u05e6\u05d1 \u05d3\u05de\u05d5')<0,'бейдж מצב דמו отсутствует у всех (подсветка имени вместо бейджа)');
