...
cd "$(dirname "$0")" || exit 1
CLIENT="appharness sorttest rtltest journey revtest edgetests drawtest drawbig autofill loginstest leaguetest fxfilter clientsched seasontests deltests logtest logfix seentest admintest archivetest cashlogtest statstest automarktest noticetest integritytest maxplayerstest intgautotest critpopuptest xsstest logunifytest legacyclient betcoltest dash365test betmathtest features2test drawpubtest histtest moneyfuzz datalink demoscan feecur resettest resetbaltest logcopytest badgetest owneriotest cachetest demotetest tgtest demobet betrowtest demohltest roundpayout gamescore"
SRV="schedtest paritytest legacytest srvlog drawpubsrv srvfuzz srvpurge srvidle srvdemote srvoverbet demobet gamescoresrv"
EXTRA_CLIENT="$1"
EXTRA_SRV="$2"
